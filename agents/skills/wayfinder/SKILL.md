...
<!-- the index contains one line per resolved ticket: enough to judge relevance, then zoom the link for the detail the issue holds -->
...
Every ticket is either **HITL**, where a human participates and speaks for themselves, or **AFK**, driven by the agent alone. A HITL ticket only resolves through that live exchange; the agent never stands in for the human's side of it (a grilling agent that answers its own questions has broken this).
...
- **Prototype** (HITL): Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to, such as an outline, a rough take, a stub, or UI/logic code via the /prototype skill. Links the prototype as an asset. Use when "how should it look" or "how should it behave" is the key question.
- **Grilling** (HITL): Conversation. The default case. Always invoke the `/grilling` and `/domain-modeling` skills.
- **Task** (HITL or AFK): Manual work that must happen before a *decision* can be made. There is nothing to decide, prototype, or research, but the discussion is blocked until the task is done. Signing up for a service so its API can be judged, provisioning access, moving data so its shape can be seen. This is the one type that *does* rather than decides. It earns its place by unblocking a decision, not by delivering the destination. The agent drives it alone where it can (AFK); otherwise it hands the human a precise checklist (HITL). Resolved when the work is done; the answer records what was done and any resulting facts (credentials location, new URLs, row counts) later tickets depend on.
...
2. **Map the frontier.** Grill again, **breadth-first** this time: fan out across the whole space rather than deep on any one thread, surfacing the open decisions and the first steps takeable now. If the way to the destination is already clear and the whole journey fits in one session, you do not need a map. Stop and ask the user how they'd like to proceed.
...
4. **Create the tickets you can specify now**, then wire blocking edges after every issue has an identity. Everything you can't yet specify stays in the fog under **Not yet specified**.
...
3. Resolve it. **Zoom as needed** by reading related ticket bodies on demand and invoke the skills named in `## Notes`. Use `/grilling` and `/domain-modeling` for unresolved human decisions.
